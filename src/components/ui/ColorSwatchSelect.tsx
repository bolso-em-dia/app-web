...
type RenderedColorOption = ColorOption | { value: ""; label: string };
...
export default function ColorSwatchSelect({ id, value, options, clearLabel, onChange }: ColorSwatchSelectProps) {
  ...
  const renderedOptions = useMemo<RenderedColorOption[]>(() => [{ value: "", label: clearLabel }, ...options], [clearLabel, options]);
      ...
      {isOpen ? (
        <div className={styles.dropdown} id={listboxId} role="listbox">
          {renderedOptions.map((option, optionIndex) => {
            const textColor = option.value ? getReadableTextColor(option.value) : null;
            ...
            return (
              <button
                ...
                role="option"
                style={option.value ? { backgroundColor: option.value, color: textColor ?? undefined } : undefined}
