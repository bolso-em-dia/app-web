export const ptBRMessages = {
  "app.brand": "Bolso em Dia",
  "app.loadingSession": "Carregando sessão",
  "appVersion.web": "web {{version}}",
  "appVersion.api": "api {{version}}",
  "navigation.aria": "Navegação principal",
  "navigation.management": "Gestão",
  "navigation.dashboard": "Visão geral",
  "navigation.budgets": "Orçamentos",
  "navigation.fixedTransactions": "Transações fixas",
  "navigation.transactions": "Transações",
  "navigation.family": "Família",
  "navigation.categories": "Categorias",
  "navigation.accounts": "Contas/Cartões",
  "common.loading": "Carregando...",
  "common.rows": "Linhas",
  "common.all": "Todos",
  "common.active": "Ativo",
  "common.archived": "Arquivado",
  "common.previous": "Anterior",
  "common.next": "Próxima",
  "common.cancel": "Cancelar",
  "common.close": "Fechar",
  "common.closeSearch": "Fechar busca",
  "common.create": "Criar",
  "common.save": "Salvar",
  "common.archive": "Arquivar",
  "common.delete": "Excluir",
  "common.retry": "Tentar novamente",
  "common.saveChanges": "Salvar",
  "common.closeDrawer": "Fechar painel",
  "common.range": "{{start}}-{{end}} de {{total}}",
  "common.pageOf": "Página {{page}} de {{total}}",
  "common.loadedItems": "{{loaded}} de {{total}} itens",
  "common.loadingMore": "Carregando mais...",
  "common.scrollToLoadMore": "Role para carregar mais",
  "common.allItemsLoaded": "Todos os itens carregados",
  "common.signOut": "Sair",
  "common.sessionExpired": "Sua sessão expirou. Entre novamente para continuar.",
  "common.name": "Nome",
  "common.email": "E-mail",
  "common.role": "Papel",
  "common.menu": "Menu",
  "common.search": "Buscar",
  "common.status": "Status",
  "common.type": "Tipo",
  "common.month": "Mês",
  "common.account": "Conta/Cartão",
  "common.category": "Categoria",
  "common.categories": "Categorias",
  "common.member": "Membro",
  "common.filters": "Filtros",
  "common.sort": "Ordenar",
  "common.sortWithValue": "Ordenar: {{value}}",
  "common.clearFilters": "Limpar filtros",
  "common.referenceMonth": "Mês de referência",
  "common.previousMonth": "Mês anterior",
  "common.nextMonth": "Próximo mês",
  "common.ownership": "Titularidade",
  "common.selectAccount": "Selecione uma conta/cartão",
  "common.selectCategory": "Selecione uma categoria",
  "common.selectCategories": "Selecione categorias",
  "common.selectMember": "Selecione um membro",
  "common.clearSelection": "Limpar seleção",
  "common.allTypes": "Todos os tipos",
  "common.allAccounts": "Todas as contas",
  "common.allCategories": "Todas as categorias",
  "common.allMembers": "Todos os membros",
  "common.allOwnerships": "Todas as titularidades",
  "roles.ADMIN": "Administrador",
  "roles.USER": "Usuário",
  "accountTypes.CHECKING": "Conta corrente",
  "accountTypes.SAVINGS": "Poupança",
  "accountTypes.CREDIT_CARD": "Cartão de crédito",
  "accountTypes.INVESTMENT": "Investimento",
  "budgetTypes.GLOBAL": "Global",
  "budgetTypes.ALLOWANCE": "Mesada",
  "transactionTypes.INCOME": "Receita",
  "transactionTypes.EXPENSE": "Despesa",
  "ownershipTypes.SHARED": "Compartilhado",
  "ownershipTypes.INDIVIDUAL": "Individual",
  "validation.requiredName": "Nome é obrigatório.",
  "validation.nameMax120": "O nome deve ter no máximo 120 caracteres.",
  "validation.requiredType": "O tipo é obrigatório.",
  "validation.requiredRole": "O papel é obrigatório.",
  "validation.requiredEmail": "E-mail é obrigatório.",
  "validation.invalidEmail": "Informe um e-mail válido.",
  "validation.emailMax160": "O e-mail deve ter no máximo 160 caracteres.",
  "validation.requiredPassword": "Senha é obrigatória.",
  "validation.passwordMin8": "A senha deve ter pelo menos 8 caracteres.",
  "validation.passwordMax72": "A senha deve ter no máximo 72 caracteres.",
  "validation.requiredCategory": "A categoria é obrigatória.",
  "validation.requiredAccount": "A conta é obrigatória.",
  "validation.requiredOwnership": "A titularidade é obrigatória.",
  "validation.requiredDescription": "A descrição é obrigatória.",
  "validation.descriptionMax160": "A descrição deve ter no máximo 160 caracteres.",
  "validation.amountPositive": "O valor deve ser maior que zero.",
  "validation.invalidNumber": "Informe um número válido.",
  "validation.requiredTransactionDate": "A data da transação é obrigatória.",
  "validation.installmentCountInteger": "A quantidade de parcelas deve ser um número inteiro.",
  "validation.installmentCountRange": "A quantidade de parcelas deve estar entre 2 e 120.",
  "validation.requiredInstallmentCount": "A quantidade de parcelas é obrigatória.",
  "validation.requiredIndividualMember": "O membro é obrigatório para transações individuais.",
  "validation.requiredAllowanceOwner": "O membro dono é obrigatório para budgets de mesada.",
  "validation.requiredGlobalBudgetCategory": "Selecione pelo menos uma categoria para budgets globais.",
  "validation.monthlyLimitPositive": "O limite mensal deve ser maior que zero.",
  "validation.iconMax80": "O ícone deve ter no máximo 80 caracteres.",
  "validation.colorMax20": "A cor deve ter no máximo 20 caracteres.",
  "validation.requiredReplacementCategory": "A categoria substituta é obrigatória.",
  "validation.brandMax80": "A bandeira deve ter no máximo 80 caracteres.",
  "validation.requiredCreditCardBrand": "A bandeira é obrigatória para cartões de crédito.",
  "validation.requiredCreditCardClosingDay": "O dia de fechamento é obrigatório para cartões de crédito.",
  "validation.requiredCreditCardDueDay": "O dia de vencimento é obrigatório para cartões de crédito.",
  "validation.dueDayInteger": "O dia de vencimento deve ser um número inteiro.",
  "validation.dueDayRange": "O dia de vencimento deve estar entre 1 e 31.",
  "validation.dayInteger": "O dia deve ser um número inteiro.",
  "validation.dayRange": "O dia deve estar entre 1 e 31.",
  "login.title": "Entrar",
  "login.subtitle": "Use a conta administrativa inicial para acessar o sistema.",
  "login.error": "Não foi possível entrar. Verifique o e-mail e a senha.",
  "login.submit": "Entrar",
  "error.validationFailed": "Verifique os campos destacados.",
  "error.invalidRequestBody": "Os dados enviados estão incorretos.",
  "error.unrecognizedField": "Há um campo não reconhecido nos dados.",
  "error.invalidParameter": "Valor inválido.",
  "error.invalidPageSize": "Número de itens por página inválido.",
  "error.dueDayRequired": "O dia de vencimento é obrigatório.",
  "error.fieldCannotBeEmpty": "Este campo não pode ficar vazio.",
  "error.notAuthenticated": "Você precisa entrar novamente.",
  "error.invalidCredentials": "E-mail ou senha inválidos.",
  "error.sessionExpired": "Sessão expirada. Entre novamente.",
  "error.accountDeactivated": "Conta desativada.",
  "error.authenticationRequired": "Você precisa estar autenticado.",
  "error.accessDenied": "Acesso negado.",
  "error.accountNotFound": "Conta não encontrada.",
  "error.budgetNotFound": "Orçamento não encontrado.",
  "error.categoryNotFound": "Categoria não encontrada.",
  "error.memberNotFound": "Membro da família não encontrado.",
  "error.fixedTransactionNotFound": "Transação fixa não encontrada.",
  "error.transactionNotFound": "Transação não encontrada.",
  "error.noExchangeRateData": "Nenhuma cotação disponível.",
  "error.emailInUse": "Este e-mail já está em uso.",
  "error.duplicateAllowance": "Já existe uma mesada para este membro.",
  "error.concurrentModification": "Este item foi modificado por outra pessoa. Recarregue a página.",
  "error.transactionDateConfirmationMismatch": "A data atual confirmada não bate com a transação salva.",
  "error.newTransactionDateConfirmationMismatch": "A nova data confirmada não bate com a próxima data permitida.",
  "error.archiveBeforeAccountCreation": "Mês de arquivamento anterior à criação da conta.",
  "error.creditCardRequiresDays": "Cartão de crédito precisa dos dias de fechamento e vencimento.",
  "error.nonCreditCardDays": "Apenas cartão de crédito pode ter dia de fechamento e vencimento.",
  "error.replacementCategorySame": "Categoria substituta deve ser diferente da arquivada.",
  "error.replacementCategoryInactive": "Categoria substituta precisa estar ativa.",
  "error.archiveBeforeCategoryCreation": "Mês de arquivamento anterior à criação da categoria.",
  "error.archiveBeforeBudgetCreation": "Mês de arquivamento anterior à criação do orçamento.",
  "error.allowanceRequiresOwner": "Mesada precisa de um membro como dono.",
  "error.globalBudgetRequiresCategory": "Orçamento global precisa de pelo menos uma categoria.",
  "error.individualRequiresMember": "Transação individual precisa de um membro.",
  "error.individualRequiresAllowance": "É preciso ter uma mesada válida para o membro selecionado.",
  "error.installmentCountRange": "Parcelas devem ser entre 1 e 120.",
  "error.installmentPlanTooLong": "Parcelamento não pode ultrapassar 2 anos.",
  "error.exchangeRateUnavailable": "Cotação não disponível no momento.",
  "error.foreignCurrencyDisabled": "Suporte a moedas estrangeiras está desativado.",
  "error.incorrectCurrentPassword": "Senha atual incorreta.",
  "error.passwordConfirmationMismatch": "A confirmação da senha não confere.",
  "error.unsupportedLocale": "Idioma não suportado.",
  "error.defaultAccountInactive": "Conta padrão precisa estar ativa no mês atual.",
  "error.dayOutOfRange": "O dia deve ser entre 1 e 31.",
  "error.referenceMonthPolicyNotAllowed": "A política de mês de referência não é permitida para esta transação.",
  "error.moveToNextMonthNotAllowed": "Só transações manuais ou parceladas podem ser movidas para o mês seguinte.",
  "error.referenceMonthPolicyUpdateNotAllowed": "Só despesas manuais em cartão permitem alterar a política de mês de referência.",
  "error.moveTransactionDateFutureScopeNotAllowed": "Mover parcelas futuras só é permitido para transações parceladas.",
  "error.unexpected": "Erro inesperado. Tente novamente.",
  "error.exchangeRateFetchFailed": "Não foi possível obter a cotação.",
  "error.exchangeRateServiceDown": "Serviço de cotação indisponível.",
  "error.validationFallback": "Verifique os dados e tente novamente.",
  "error.authFallback": "Sessão expirada. Entre novamente.",
  "error.forbiddenFallback": "Acesso negado.",
  "error.notFoundFallback": "Item não encontrado.",
  "error.conflictFallback": "Conflito. O item pode já existir ou ter sido modificado.",
  "error.businessRuleFallback": "Operação não permitida.",
  "error.internalFallback": "Erro inesperado. Tente novamente.",
  "error.externalServiceFallback": "Serviço indisponível no momento.",
  "errorBoundary.title": "Algo deu errado",
  "errorBoundary.message": "Um erro inesperado ocorreu. Você pode tentar recarregar esta página ou voltar ao início.",
  "errorBoundary.details": "Detalhes técnicos",
  "errorBoundary.retry": "Tentar novamente",
  "errorBoundary.goToDashboard": "Ir para a Visão geral",
  "exchangeRate.updated": "Cotação atualizada",
  "exchangeRate.staleTooltip": "Cotação desatualizada. Última atualização pode estar incorreta.",
  "exchangeRate.fetchError": "Erro ao buscar cotação. Último valor disponível.",
  "exchangeRate.refresh": "Atualizar cotação",
  "exchangeRate.display": "US$ 1 = {{rate}}",
  "exchangeRate.reference": "{{amount}} (cot. {{rate}})",
  "home.title": "Visão geral",
  "home.referenceMonth": "Mês de referência",
  "home.income": "Receitas",
  "home.expense": "Despesas",
  "home.expenseWithBudgets": "Despesas + Orçamentos",
  "home.balance": "Saldo realizado",
  "home.loading": "Carregando dashboard",
  "home.unavailable": "Dashboard indisponível",
  "home.error": "Não foi possível carregar o dashboard agora.",
  "home.session": "Sessão",
  "home.budgets": "Orçamentos",
  "home.availableBalance": "Saldo livre",
  "home.reservedBudgetAmount": "Reservado em orçamentos",
  "home.considerBudgetsInBalance": "Considerar orçamentos no saldo",
  "settings.title": "Configurações",
  "settings.account.label": "Conta padrão",
  "settings.account.description": "Conta inicial pré-selecionada em formulários com conta.",
  "settings.account.empty": "Nenhuma conta padrão",
  "settings.locale.label": "Idioma",
  "settings.locale.description": "Idioma usado ao carregar a aplicação.",
  "settings.locale.pt-BR": "Português (Brasil)",
  "settings.locale.en-US": "Inglês (Estados Unidos)",
  "settings.balance.label": "Modo inicial do saldo livre",
  "settings.balance.description": "Define se a Home inicia considerando os orçamentos no saldo livre.",
  "settings.balance.withBudgets": "Considera orçamentos",
  "settings.balance.withoutBudgets": "Ignora orçamentos",
  "settings.foreignCurrency.label": "Moedas estrangeiras",
  "settings.foreignCurrency.description": "Habilita suporte a Dólar (USD) com cotação em tempo real.",
  "settings.foreignCurrency.enabled": "Habilitado",
  "settings.foreignCurrency.disabled": "Desabilitado",
  "settings.loading": "Carregando configurações",
  "settings.formTitle": "Preferências pessoais",
  "settings.formSubtitle": "Ajuste como a aplicação carrega para a sua conta.",
  "settings.error": "Não foi possível carregar as configurações.",
  "settings.saveError": "Não foi possível salvar as configurações.",
  "settings.saveSuccess": "Configurações salvas.",
  "settings.save": "Salvar",
  "settings.password.title": "Senha",
  "settings.password.subtitle": "Altere sua senha informando a atual e confirmando a nova.",
  "settings.password.current": "Senha atual",
  "settings.password.new": "Nova senha",
  "settings.password.confirm": "Confirmar nova senha",
  "settings.password.save": "Salvar",
  "settings.password.success": "Senha atualizada.",
  "settings.password.saveError": "Não foi possível atualizar a senha.",
  "settings.password.confirmationMismatch": "A confirmação da senha não confere.",
  "settings.password.firstAccessTitle": "Defina uma nova senha",
  "settings.password.firstAccessSubtitle": "O acesso inicial do administrador exige a troca da senha antes de continuar.",
  "settings.password.firstAccessSave": "Salvar nova senha",
  "home.consumedOfLimit": "Usado {{consumed}} de {{limit}}",
  "home.recentTransactions": "Lançamentos recentes",
  "home.categoryBreakdown": "Gastos por categoria",
  "home.allowanceFor": "Mesada de {{name}}",
  "home.linkedCategories": "{{count}} categorias vinculadas",
  "home.usedAmount": "{{amount}} consumido",
  "categories.title": "Categorias",
  "categories.loading": "Carregando categorias",
  "categories.error": "Não foi possível carregar as categorias",
  "categories.saveError": "Não foi possível salvar a categoria.",
  "categories.archiveError": "Não foi possível arquivar a categoria.",
  "categories.new": "Nova",
  "categories.searchPlaceholder": "Buscar categorias",
  "categories.empty": "Nenhuma categoria encontrada para os filtros atuais.",
  "categories.noIcon": "Sem ícone",
  "categories.noColor": "Sem cor",
  "categories.iconPreviewNone": "Sem preview",
  "categories.newTitle": "Nova categoria",
  "categories.detailsTitle": "Detalhes da categoria",
  "categories.create": "Criar categoria",
  "categories.icon": "Ícone",
  "categories.color": "Cor",
  "categories.archiveMonth": "Mês de arquivamento",
  "categories.archiveTitle": "Arquivar categoria",
  "categories.archiveSubtitle": "Categorias arquivadas exigem uma substituta para uso futuro.",
  "categories.replacementCategory": "Categoria substituta",
  "categories.selectReplacement": "Selecione uma substituta",
  "family.title": "Família",
  "family.loading": "Carregando membros da família",
  "family.error": "Não foi possível carregar os membros da família",
  "family.saveError": "Não foi possível salvar o membro da família.",
  "family.statusError": "Não foi possível atualizar o status do membro.",
  "family.new": "Novo",
  "family.searchPlaceholder": "Buscar por nome ou e-mail",
  "family.empty": "Nenhum membro da família encontrado para os filtros atuais.",
  "family.newTitle": "Novo membro",
  "family.detailsTitle": "Detalhes do membro",
  "family.password": "Senha",
  "family.passwordOptional": "Senha (opcional)",
  "family.create": "Criar membro",
  "family.memberStatus": "Status do membro",
  "family.archiveMember": "Arquivar membro",
  "family.restoreMember": "Reativar membro",
  "family.sort.nameAsc": "Nome A → Z",
  "family.sort.nameDesc": "Nome Z → A",
  "family.sort.emailAsc": "E-mail A → Z",
  "family.sort.emailDesc": "E-mail Z → A",
  "accounts.title": "Contas/Cartões",
  "accounts.loading": "Carregando contas",
  "accounts.error": "Não foi possível carregar as contas",
  "accounts.saveError": "Não foi possível salvar a conta.",
  "accounts.archiveError": "Não foi possível arquivar a conta.",
  "accounts.new": "Nova",
  "accounts.searchPlaceholder": "Buscar contas e cartões",
  "accounts.empty": "Nenhuma conta encontrada para os filtros atuais.",
  "accounts.newTitle": "Nova conta/cartão",
  "accounts.detailsTitle": "Detalhes da conta/cartão",
  "accounts.create": "Criar conta",
  "accounts.brand": "Bandeira",
  "accounts.color": "Cor",
  "accounts.closingDay": "Dia de fechamento",
  "accounts.dueDay": "Dia de vencimento",
  "accounts.archiveTitle": "Arquivar conta",
  "accounts.archiveMonth": "Mês de arquivamento",
  "accounts.archived": "Conta arquivada",
  "accounts.placeholder": "Conta principal",
  "accounts.currency": "Moeda",
  "accounts.billingCycle": "Fecha {{closingDay}} · Vence {{dueDay}}",
  "accounts.sort.nameAsc": "Nome A → Z",
  "accounts.sort.nameDesc": "Nome Z → A",
  "accounts.sort.typeAsc": "Tipo A → Z",
  "accounts.sort.typeDesc": "Tipo Z → A",
  "currencies.BRL": "Real (BRL)",
  "currencies.USD": "Dólar (USD)",
  "ui.colors.red": "Vermelho",
  "ui.colors.orange": "Laranja",
  "ui.colors.amber": "Âmbar",
  "ui.colors.yellow": "Amarelo",
  "ui.colors.lime": "Lima",
  "ui.colors.green": "Verde",
  "ui.colors.emerald": "Esmeralda",
  "ui.colors.teal": "Turquesa",
  "ui.colors.cyan": "Ciano",
  "ui.colors.sky": "Azul claro",
  "ui.colors.blue": "Azul",
  "ui.colors.indigo": "Índigo",
  "ui.colors.violet": "Violeta",
  "ui.colors.purple": "Roxo",
  "ui.colors.fuchsia": "Fúcsia",
  "ui.colors.pink": "Rosa",
  "ui.colors.rose": "Rose",
  "ui.colors.stone": "Pedra",
  "ui.colors.gray": "Cinza",
  "ui.colors.graphite": "Grafite",
  "ui.icons.shoppingCart": "Compras",
  "ui.icons.car": "Transporte",
  "ui.icons.home": "Casa",
  "ui.icons.health": "Saúde",
  "ui.icons.work": "Trabalho",
  "ui.icons.payment": "Pagamento",
  "ui.icons.travel": "Viagem",
  "ui.icons.gifts": "Presentes",
  "ui.icons.education": "Educação",
  "ui.icons.leisure": "Lazer",
  "ui.icons.technology": "Tecnologia",
  "ui.icons.games": "Jogos",
  "ui.icons.clothes": "Roupas",
  "ui.icons.school": "Escola",
  "ui.icons.fitness": "Fitness",
  "ui.icons.pharmacy": "Farmácia",
  "ui.icons.personalCare": "Cuidados pessoais",
  "ui.icons.loans": "Empréstimos",
  "ui.icons.groceries": "Mercado",
  "ui.icons.pets": "Pets",
  "ui.icons.services": "Serviços",
  "fixedTransactions.title": "Transações fixas",
  "fixedTransactions.loading": "Carregando transações fixas",
  "fixedTransactions.error": "Não foi possível carregar as transações fixas",
  "fixedTransactions.saveError": "Não foi possível salvar a transação fixa.",
  "fixedTransactions.archiveError": "Não foi possível arquivar a transação fixa.",
  "fixedTransactions.new": "Nova",
  "fixedTransactions.searchPlaceholder": "Buscar transações fixas",
  "fixedTransactions.empty": "Nenhuma transação fixa encontrada para os filtros atuais.",
  "fixedTransactions.newTitle": "Nova transação fixa",
  "fixedTransactions.detailsTitle": "Detalhes da transação fixa",
  "fixedTransactions.create": "Criar transação fixa",
  "fixedTransactions.amount": "Valor",
  "fixedTransactions.receiptDay": "Dia do recebimento",
  "fixedTransactions.dueOnDay": "Vence dia {{day}}",
  "fixedTransactions.receivesOnDay": "Recebe dia {{day}}",
  "fixedTransactions.archiveMonth": "Mês de arquivamento",
  "fixedTransactions.archived": "Transação fixa arquivada",
  "fixedTransactions.deleteAction": "Excluir transação fixa",
  "fixedTransactions.deleteError": "Não foi possível excluir a transação fixa.",
  "fixedTransactions.sort.nameAsc": "Nome A → Z",
  "fixedTransactions.sort.nameDesc": "Nome Z → A",
  "fixedTransactions.sort.amountDesc": "Valor maior → menor",
  "fixedTransactions.sort.amountAsc": "Valor menor → maior",
  "fixedTransactions.sort.dueDayAsc": "Dia recorrente menor → maior",
  "fixedTransactions.sort.dueDayDesc": "Dia recorrente maior → menor",
  "budgets.title": "Orçamentos",
  "budgets.loading": "Carregando orçamentos",
  "budgets.error": "Não foi possível carregar os orçamentos",
  "budgets.detailsError": "Não foi possível carregar os detalhes do orçamento.",
  "budgets.saveError": "Não foi possível salvar o orçamento.",
  "budgets.archiveError": "Não foi possível arquivar o orçamento.",
  "budgets.new": "Novo",
  "budgets.searchPlaceholder": "Buscar orçamentos",
  "budgets.empty": "Nenhum orçamento encontrado para os filtros atuais.",
  "budgets.allowanceFor": "Mesada de {{name}}",
  "budgets.linkedCategories": "{{count}} categorias vinculadas",
  "budgets.consumedOfLimit": "Usado {{consumed}}",
  "budgets.archivedFrom": "Arquivado a partir de {{month}}",
  "common.archivedFrom": "Arquivado a partir de {{month}}",
  "budgets.newTitle": "Novo orçamento",
  "budgets.detailsTitle": "Detalhes do orçamento",
  "budgets.create": "Criar orçamento",
  "budgets.monthlyLimit": "Limite mensal",
  "budgets.ownerMember": "Membro dono",
  "budgets.linkedCategoriesTitle": "Categorias vinculadas",
  "budgets.globalHelper": "Orçamentos globais consomem categorias de despesas compartilhadas.",
  "budgets.currentImpact": "Impacto no mês atual",
  "budgets.currentImpactSubtitle": "Consumo e lançamentos correspondentes de {{month}}.",
  "budgets.loadingDetails": "Carregando detalhes do orçamento",
  "budgets.limit": "Limite",
  "budgets.consumed": "Consumido",
  "budgets.remaining": "Restante",
  "budgets.categoryBreakdown": "Consumo por categoria",
  "budgets.noCategoryConsumption": "Não houve consumo por categoria neste orçamento no mês atual.",
  "budgets.matchedTransactions": "Lançamentos correspondentes",
  "budgets.noMatchedTransactions": "Nenhum lançamento correspondeu a este orçamento no mês atual.",
  "budgets.noSharedCategories": "Este orçamento de mesada não usa categorias compartilhadas.",
  "budgets.selectToReview": "Selecione um orçamento para revisar os detalhes do mês atual.",
  "budgets.archiveTitle": "Arquivar orçamento",
  "budgets.archiveSubtitle": "Pare de usar este orçamento a partir de um mês futuro.",
  "budgets.archiveMonth": "Mês de arquivamento",
  "budgets.archived": "Orçamento arquivado",
  "budgets.sort.nameAsc": "Nome A → Z",
  "budgets.sort.nameDesc": "Nome Z → A",
  "budgets.sort.monthlyLimitDesc": "Teto maior → menor",
  "budgets.sort.monthlyLimitAsc": "Teto menor → maior",
  "budgets.sort.remainingAmountDesc": "Saldo maior → menor",
  "budgets.sort.remainingAmountAsc": "Saldo menor → maior",

  "transactions.title": "Transações",
  "transactions.searchPlaceholder": "Buscar por descrição",
  "transactions.loading": "Carregando transações",
  "transactions.error": "Não foi possível carregar as transações",
  "transactions.categoryRefreshError": "Não foi possível atualizar as opções de categoria.",
  "transactions.saveError": "Não foi possível salvar a transação.",
  "transactions.deleteError": "Não foi possível excluir a transação.",
  "transactions.new": "Nova",
  "transactions.empty": "Nenhuma transação encontrada para os filtros atuais.",
  "transactions.fixed": "Fixo",
  "transactions.projected": "Prevista",
  "transactions.newTitle": "Nova transação",
  "transactions.detailsTitle": "Detalhes da transação",
  "transactions.create": "Criar transação",
  "transactions.save": "Salvar",
  "transactions.saveAndCreateNew": "Salvar e criar novo",
  "transactions.description": "Descrição",
  "transactions.amount": "Valor",
  "transactions.transactionDate": "Data da transação",
  "transactions.creditCardNextMonthToggle": "Considerar no mês seguinte",
  "transactions.creditCardNextMonthTooltip":
    "Compras após o fechamento do cartão normalmente entram na fatura do mês seguinte. Marcado mantém esse comportamento; desmarcado força considerar neste mês.",
  "transactions.installmentToggle": "Parcelado",
  "transactions.installmentCount": "Quantidade de parcelas",
  "transactions.moveToNextMonth": "Mover para o mês seguinte",
  "transactions.moveDateTitle": "Mover data da transação",
  "transactions.moveDateSubtitle": "Confirme a mudança da data da transação selecionada.",
  "transactions.moveDateCurrent": "Data atual: {{date}}",
  "transactions.moveDateNew": "Nova data: {{date}}",
  "transactions.moveDateFutureToggle": "Mover próximas parcelas também",
  "transactions.moveDateInstallmentHint": "As parcelas futuras deste grupo também serão movidas para o mês seguinte.",
  "transactions.moveDateAction": "Mover data",
  "transactions.moveError": "Não foi possível mover a data da transação.",
  "transactions.deleteTitle": "Excluir transação",
  "transactions.deleteSubtitle": "Confirme a exclusão da transação selecionada ou das parcelas restantes do grupo.",
  "transactions.deleteSingleSubtitle": "Confirme a exclusão da transação selecionada.",
  "transactions.deleteScope": "Escopo da exclusão",
  "transactions.deleteScope.single": "Somente esta parcela",
  "transactions.deleteScope.future": "Esta e futuras parcelas",
  "transactions.deleteScope.all": "Grupo inteiro de parcelas",
  "transactions.noInstallmentGroup": "Esta transação não faz parte de um grupo de parcelamento.",
  "transactions.deleteAction": "Excluir transação",
  "transactions.closeDeleteConfirm": "Fechar confirmação de exclusão",
  "transactions.sort.transactionDateDesc": "Data mais recente → mais antiga",
  "transactions.sort.transactionDateAsc": "Data mais antiga → mais recente",
  "transactions.sort.amountDesc": "Valor maior → menor",
  "transactions.sort.amountAsc": "Valor menor → maior",
  "transactions.sort.descriptionAsc": "Descrição A → Z",
  "transactions.sort.descriptionDesc": "Descrição Z → A",
  "categories.sort.nameAsc": "Nome A → Z",
  "categories.sort.nameDesc": "Nome Z → A",
  "confirmations.archiveBudget": "Tem certeza que deseja arquivar este orçamento? Ele deixará de aparecer a partir do mês atual.",
  "confirmations.archiveCategory":
    "Tem certeza que deseja arquivar esta categoria? A categoria substituta será usada a partir do mês atual.",
  "confirmations.archiveAccount":
    "Tem certeza que deseja arquivar esta conta? Transações passadas preservam o vínculo e a conta não aparecerá em novos lançamentos.",
  "confirmations.archiveFixedExpense":
    "Tem certeza que deseja arquivar esta transação fixa? As transações já materializadas serão preservadas, mas o modelo deixará de gerar novas.",
  "confirmations.deleteFixedExpense":
    "Tem certeza que deseja excluir esta transação fixa? As transações materializadas do mês corrente em diante serão removidas. Transações de meses passados serão preservadas.",
  "confirmations.archiveMember": "Tem certeza que deseja arquivar este membro? Ele não poderá mais fazer login.",
  "confirmations.restoreMember": "Tem certeza que deseja reativar este membro?",
} as const;

export type MessageKey = keyof typeof ptBRMessages;
